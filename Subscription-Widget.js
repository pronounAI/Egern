 ...
 *   WIDGET_STYLE=glass (default) | classic
 *     - glass:   根容器不设置背景色，桌面壁纸直接透出来
 *     - classic: 保留原来的浅色/深色纯色背景
 */
...
function statusOf(data) {
  ...
  if (data.mode === 'error') {
    return { label: '更新失败', icon: 'exclamationmark.circle.fill', bgColor: 'rgba(214, 69, 69, 0.35)', iconColor: '#FF626A' };
  }
  if (data.mode === 'stale') {
    return { label: '数据延迟', icon: 'clock.arrow.circlepath', bgColor: 'rgba(255, 190, 63, 0.3)', iconColor: '#FFBE3F' };
  }
...
function fallbackGauge(traffic, size = 90) {
  ...
  return {
    ...
    children: [
      { type: 'spacer' },
      icon('gauge.with.dots.needle.33percent', C.accent, 46),
      text(usedPercent == null ? '--' : `${usedPercent.toFixed(0)}%`, 14, C.text, 'bold', {
...
function gaugeView(data, traffic, size = 86) {
        ...
        children: [
          {
            ...
            flex: 1,
            children: [
              text(formatBytes(traffic.remaining), 26, C.text, 'bold', {
                font: { size: 26, weight: 'bold', family: 'Menlo' },
                minScale: 0.62
              }),
              ...
              { type: 'spacer' },
              text(data.name, 9, C.dim, 'medium', { minScale: 0.7 })
            ]
          },
          {
            type: 'stack',
