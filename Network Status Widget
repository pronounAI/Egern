...
export default async function(ctx) {
   ...
   * ============================================================
   *
   * 所有通过 get/post/getRaw 的请求都会严格使用 POLICY。
   *
   * POLICY 为空：
   *   不指定策略，使用 Egern 默认行为
   *
  ...
  async function checkChatGPT() {
    try {
      ...
      if (appAccessible && !webAccessible) {
        return "APP";
      }
  ...
  const fmtISP = (isp) => {
    ...
    if (/移动|mobile|cmcc/i.test(s)) {
      return "中国移动";
    }

    if (/电信|telecom|chinanet/i.test(s)) {
      return "中国电信";
    }

    if (/联通|unicom/i.test(s)) {
      return "中国联通";
    }

    if (/广电|broadcast|cbn/i.test(s)) {
      return "中国广电";
    }
  ...
  if (lIp === "获取失败") {
    try {
      const res126 = await ctx.http.get(
      ...
      if (body126?.result) {
        ...
        lLoc =
          `🇨🇳 ${body126.result.province || ""} ${body126.result.city || ""}`
            .trim();

        lIsp = fmtISP(
          body126.result.operator ||
          body126.result.company
        );
      }
    } catch (e) {}
  }
  ...
  let riskIPPureTxt = "低危 (0)";
  let riskIPPureCol = C_GREEN;
  let ippSev = 0;
  ...
  try {
    ...
    if (risk !== null) {
      if (risk >= 80) {
        ...
        riskIPPureTxt =
          `高危 (${risk})`;
        riskIPPureCol = C_ORANGE;
        ippSev = 3;
      } else if (risk >= 40) {
        riskIPPureTxt =
          `中等 (${risk})`;
        riskIPPureCol = C_YELLOW;
        ippSev = 1;
      } else {
        riskIPPureTxt =
          `低危 (${risk})`;
  ...
  let riskIpapiTxt = "低危 (0%)";
  let riskIpapiCol = C_GREEN;
  let apiSev = 0;
  ...
  try {
    ...
    if (ipData.query) {
       ...
       * 同样严格使用 POLICY
       */
      const apiRes = await getRaw(
        `https://api.ipapi.is/?q=${ipData.query}`
      );
      ...
      if (
        ...
        if (m) {
          ...
          riskIpapiCol =
            lv.includes('High') ||
            lv.includes('Very High')
              ? C_ORANGE
              : (
                  lv.includes('Elevated')
                    ? C_YELLOW
                    : C_GREEN
                );
  ...
  if (proxySuccess) {
    riskGrades.push({
      sev: ippSev,
      t: `IPPure: ${riskIPPureTxt}`
    });

    riskGrades.push({
      sev: apiSev,
      t: `ipapi: ${riskIpapiTxt}`
    });
  ...
  function ScoreRow(grade) {
    ...
    return {
      ...
      children: [
        {
          type: 'image',
          src: `sf-symbol:${sevIcon(grade.sev)}`,
          color: col,
          width: SMALL_ICON,
          height: SMALL_ICON
        },
